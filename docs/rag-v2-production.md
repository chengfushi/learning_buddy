...
RAG v2 将正文、摘要、候选问题和图片说明分开存为 `body/summary/question/image` 信号，不向每个正文块重复拼接摘要。查询时执行向量 Top-30 与全文/Trigram Top-30，使用 RRF(k=60) 合并为 20 条；`qwen3-rerank` 保留 Top-8，超时或返回重复、非有限、数量不足的结果时回退 RRF。父资料扩展再次鉴权，最多选择 3 份资料、8 个正文块和 12k Token。Backend 只接受实际上下文中存在的 chunk ID，并用可信数据库字段重建引用。
