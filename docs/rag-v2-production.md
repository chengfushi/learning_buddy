...
RAG v2 将正文、摘要、候选问题和图片说明分开存为 `body/summary/question/image` 信号，不向每个正文块重复拼接摘要。查询时执行向量 Top-30 与全文/Trigram Top-30，使用 RRF(k=60) 合并为 20 条；`qwen3-rerank` 保留 Top-8，送入模型的单块内容按 `RERANK_MAX_DOCUMENT_TOKENS`（默认 4000）保守截断，超时或返回重复、非有限、数量不足的结果时回退 RRF。父资料扩展再次鉴权，最多选择 3 份资料、8 个正文块和 12k Token。Backend 只接受实际上下文中存在的 chunk ID，并用可信数据库字段重建引用。

Query Rewrite 仅在存在对话历史且问题包含明确指代或短省略问法时触发；已经自包含的技术问题直接进入检索，避免额外模型调用和语义漂移。改写结果只用于召回，生成端仍回答用户的原始问题。

生成入口还会过滤空白片段；没有任何可引用正文时固定返回“当前知识库未找到依据”，不调用 Tutor，也不允许模型依靠常识补全答案。
