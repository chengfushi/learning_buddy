...
| [资料 / Materials](#3-资料--materials) | 5 | 资料 CRUD、可见性过滤、异步解析 |
...
- teacher team 学生仅可见 `shared=true` 的资料
- 私人 team owner 可见全部
- 公共库所有人可见
...
> **权限安全**：所有资料查询均经 `VisibleMaterialsScope` 过滤——`team_id IN(可见集合) AND (type<>'teacher' OR shared=true)`。该谓词是用户可见资料的唯一真源，仅在 backend repository 层构建。
...
> 📝 新创建的资料 `ParseStatus` 为 `pending`，后端异步触发 Agent 解析：`pending → parsing → done/failed`。前端可根据 `parse_status` 展示解析进度。
...
> **安全边界**：后端在调用 Agent 前计算「用户可见 team 集合 + `shared` 过滤谓词」并注入请求。Agent 仅持 `material_chunks` 的检索能力，不直接访问权限表（system-design §7.4）。
...
AI 对话（SSE 流式）。后端自动：计算可见 team 集合 → 新建或复用会话 → 加载历史消息 → 调 Agent 流式接口 → 转发 SSE 事件 → 落 assistant 消息 + token 用量。
...
> ⚠️ `Options` 为 JSONB 数组；`AnswerKey` 为正确选项标识（如 `"A"`），前端不应直接暴露给作答中的用户。
...
提交测评答案并批改。批改为**本地判定**（大小写不敏感比较 `choice` 与 `answer_key`），答对得满分 100，答错得 0 分。作答记录落 `quiz_attempts` 表（F8）。
...
| `failed` | 解析失败，`parse_error` 含原因 | 显示错误信息 + 重试入口 |
...
StorageKey, ParseStatus, ParseError, Shared, OwnerID, CreatedAt
```
...
**Exercise**
```
ID, MaterialID, Question, Options, AnswerKey, Difficulty, CreatedAt
```
