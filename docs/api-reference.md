...
```
data: {"type":"meta","session_id":"...","trace_id":"...","rewritten_query":"...","rewrite_applied":true}
...
data: {"type":"done","session_id":"...","message_id":42,"citations":[...],"stage_ms":{"retrieve":80},"degraded_stages":[]}
...
| Event Type | 说明 |
|------------|------|
| `meta` | 会话、trace 与 Query Rewrite 元数据 |
| `token` | 逐字输出文本（汉字按 3 字为粒度、英文按词/空格分割） |
| `done` | 回答已成功落库，附带正数 `message_id`、可信引用和检索诊断；这是持久化成功标志 |
| `error` | 生成中断、空回答或落库失败；该次响应不会再发送 `done` |
| `end` | SSE 传输结束；成功和失败路径都会发送，不能单独作为落库成功标志 |

Agent 提前 EOF、返回空回答或 assistant 消息落库失败时，Backend 发送 `error` 后直接 `end`。前端必须只在收到合法 `done` 时启用反馈和更新持久会话状态，且拒绝 `message_id <= 0`。
