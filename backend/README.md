...
> 智能学伴系统后端——鉴权 / Team & RBAC / 资料 CRUD / 学习记录 / Agent 网关。不负责模型推理与向量检索实现。
...
# 3. 初始化数据库（首次）
# 按顺序执行 migrations/ 下的 SQL 脚本：
#   0001_init.sql → 0002_seed.sql → 0003_fix_sequences.sql → 0004_embedding_1024.sql
...
└── internal/
    ...
    │   ├── material.go        #   资料业务（上传 / 更新 / shared 翻转 / 触发解析）
    │   ├── learning.go        #   学习记录业务
...
| `GET` | `/api/teams/:id/materials` | team 内可见资料（自动过滤 `shared`） | 所有角色 |
...
| `GET` | `/api/materials/:id/notes` | 笔记列表 | 所有角色 |
| `POST` | `/api/materials/:id/notes` | 创建笔记 | 所有角色 |
...
2. **权限集中（R2）**：`repository.VisibleTeamIDs()` + `VisibleMaterialsScope()` 是用户可见资料范围的唯一真源。任何新资料查询接口都应复用这两个方法。
3. **分层清晰**：Handler 只管校验+序列化，Service 编排业务，Repository 拼 SQL——禁止反向依赖。
4. **Agent 边界**：后端向 Agent 下发「可见 team 集合 + `shared` 过滤谓词」，Agent 仅持 `material_chunks` 只读凭证，不直接访问权限表。
