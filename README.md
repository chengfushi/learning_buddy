...
| Docker | 近期版本 | 运行应用服务与宿主机独立基础设施容器 |

### 1. 启动宿主机基础设施

```bash
# PostgreSQL、Redis、MinIO 是独立 Docker 容器，不由项目 Compose 创建或挂载。
make infra
...
项目根目录 `.env` 保存 Compose 使用的本机连接地址和 `AGENT_SHARED_SECRET`。应用容器通过 `host.docker.internal` 访问上述三个实例。
...
│   ├── e2e.sh               # P0 主流程
│   └── r2.sh                # R2 权限隔离
├── docker-compose.yml       # 三个应用服务；基础设施连接宿主机独立容器
...
# 启动开发环境（Frontend / Agent / Backend 均支持代码热更新）
make dev
